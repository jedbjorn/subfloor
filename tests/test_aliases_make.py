...
class MakeAliasContractTest(unittest.TestCase):
    ...
    def test_documented_targets_delegate_exactly_to_sc(self):
        cases = [
            # This installation is bare metal: lifecycle/session targets run
            # through the host adapter, engine updates through the pinned
            # updater (Makefile SC overrides) — not bare `./sc`.
            (("dos-enter",), "sh scripts_sc/host_sc.sh enter"),
            (("dos-e", "s=DEV1"), "sh scripts_sc/host_sc.sh enter-DEV1"),
            (("dos-admin",), "./sc admin"),
            (("dos-admin", "ARGS=--harness codex"),
             "./sc admin --harness codex"),
            (("dos-launch", "ARGS=--no-build"),
             "sh scripts_sc/host_sc.sh launch --no-build"),
            (("dos-l",), "sh scripts_sc/host_sc.sh launch"),
            (("dos-restart", "ARGS=--yes --no-build"),
             "sh scripts_sc/host_sc.sh restart --yes --no-build"),
            (("dos-r", "ARGS=--yes"), "sh scripts_sc/host_sc.sh restart --yes"),
            (("dos-down",), "sh scripts_sc/host_sc.sh down"),
            (("dos-d",), "sh scripts_sc/host_sc.sh down"),
            (("dos-update", "ARGS=--no-fetch"),
             "sh scripts_sc/update_engine.sh update --no-fetch"),
            (("dos-u",), "sh scripts_sc/update_engine.sh update"),
            ...
            (("dos-build",), "./sc build"),
            (("dos-logs",), "sh scripts_sc/host_sc.sh logs"),
