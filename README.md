...
Use Linux: Arch Linux (including CachyOS) or Ubuntu LTS, Git, curl, Python
3.14.x with `sqlite3`, and a reachable Docker daemon for the default sandbox.
Select `./sc install --runtime host` for the supervised host runtime without
Docker. On macOS or Windows, use a Linux VM; keep the checkout on guest storage.
