...
Focused runbooks under [`docs/`](docs/) cover the JSON HTTP API, harness
freshness, engine integrity, opt-in browser driving, the browser-conversation
runtime, the Sprint PR watcher, remote seats (host-owned VM and named remotes),
the optional Postgres sidecar, and the optional tailnet, PM2 and app-DB
brokers. Their applicability labels separate current operation from
architecture reference. The
