...
def _legacy_seed_tombstone_tolerance(tombstones: set[str]) -> set[str]:
    """Temporary generated-seed overlap ratified for feature 33 unit 2.

    The allowance is self-removing: once unit 4 regenerates ``0001`` without
    the three retired assets, none of their names are seed-owned and a later
    asset cannot reclaim them.
    """
    return (
...
def validate_upstream_skill_namespace(
    active_names: list[str], *, allow_legacy_seed_overlap: bool = False
) -> list[str]:
    ...
    if allow_legacy_seed_overlap:
        overlap -= _legacy_seed_tombstone_tolerance(tombstone_set)
    if overlap:
        raise ValueError(
...
def _validate_skill_specs_claimable(specs: list[dict]) -> None:
    ...
    tolerated = _legacy_seed_tombstone_tolerance(tombstones)
    claimed = sorted(
...
def main() -> int:
    ...
    validate_upstream_skill_namespace(
