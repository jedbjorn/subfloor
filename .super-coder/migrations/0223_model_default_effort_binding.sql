-- Feature #54 / spec #160 — admit model-default Sprint route bindings.
--
-- A controlled binding with requested_effort='default' pins the exact model
-- but applies no effort transport, so it carries no effort-value digest and
-- no native variant.  Rebuild sprint_participant_route_bindings so the table
-- CHECK admits exactly that shape; named-effort rows keep the 0212 rules.
-- Existing immutable rows are copied verbatim — no backfill, no rewrite.
...
-- Triggers on sprint_participants reference the rebuilt table by name; drop
-- them before the rebuild and restore them after (same pattern as 0185).
...
CREATE TABLE _sprint_participant_route_bindings_model_default (
...
INSERT INTO _sprint_participant_route_bindings_model_default (
    binding_id,
    participant_id,
    route_revision,
    contract_version,
...
ALTER TABLE _sprint_participant_route_bindings_model_default
RENAME TO sprint_participant_route_bindings;

-- DROP TABLE removed the AUTOINCREMENT ledger row; restore it so new
-- binding ids stay strictly above every preserved historical id.
