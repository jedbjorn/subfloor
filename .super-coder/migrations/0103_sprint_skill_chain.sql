-- 0103 — split sprint participation by role and lazy-load planner contingencies.
-- The steady-state planner skill owns declaration, dispatch, and event routing.
-- Recovery and close are separate planner-only skills loaded by explicit triggers.
-- The combined participant skill retires in favor of sprint_dev and sprint_review.
...
-- The old combined participant procedure is inert after this migration. Grants
-- remain as historical rows; is_deleted is the catalogue/render switch.
UPDATE skills SET is_deleted = 1 WHERE name = 'sprint';

-- Grant the chain to existing planner shells.
...
WHERE COALESCE(s.is_deleted, 0) = 0
  ...
  AND k.name IN ('sprint_orchestration',
                 'sprint_orchestration_recover',
                 'sprint_orchestration_close')
  AND k.is_deleted = 0;

-- Grant only the participant procedure each role executes.
INSERT OR IGNORE INTO shell_skills (shell_id, skill_id)
SELECT s.shell_id, k.skill_id
FROM shells s, skills k
WHERE COALESCE(s.is_deleted, 0) = 0
  AND ((s.flavor = 'dev' AND k.name = 'sprint_dev')
    OR (s.flavor = 'reviewer' AND k.name = 'sprint_review'))
  AND k.is_deleted = 0;
