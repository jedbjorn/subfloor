-- 0051 — reseed review: test doctrine — review the test diff, don't re-run the suite
--
-- The reviewer's boot doc (templates/shells/reviewer.json focus) now tells review
-- shells to stop re-running the green suite the dev pre-gated and CI runs, and to
-- review the test diff instead (coverage/quality via the test_authoring lens),
-- strengthening tests where a real gap exists — the one execution worth its cost
-- being the always-green guard. This forward-reseeds the `review` skill's Stance
-- to match: "Verify, don't trust" no longer says "Re-run the tests".
--
-- 0040 and 0049 carry the OLD "Re-run the tests" body and run after 0001, so a
-- fresh build lands the stale text; a 0001 regen alone is overridden. This forward
-- reseed (UPSERT by name; skill_id + grants preserved) carries the amended body to
