-- 0090 — seed the source-repo-only engine_surgery skill (common=0, absent from
-- every flavor template, so a fork seeds the row but never grants it), and
-- reseed BOTH sprint skills to replace three absolute rebase/ancestor rules
-- with evidence-based ones. 0089 shipped 'rebase whenever main moved' and
-- 'refuse unless main is an ancestor'; both fire on every UNRELATED merge and
-- cost cycles for no safety gain — the same over-strictness decision #56 had
-- already rejected on the carry-over rule. The test is now whether the merges
-- since touch THIS unit's files.
