-- Feature #60 / spec #167 — admit controlled DeepSeek participant routes.
--
-- DeepSeek remains exact-route-only.  Rebuild the immutable participant
-- binding table so controlled rows admit its provider-options-v1 transport;
-- harness-default/native-uncontrolled shapes remain unchanged.  Existing
-- binding bytes and provenance copy verbatim.
...
CREATE TABLE _sprint_participant_route_bindings_deepseek (
    ...
    CHECK (
      (control_state='controlled'
       ...
       AND transport=CASE harness
         ...
         WHEN 'deepseek' THEN 'deepseek-provider-options-v1'
...
INSERT INTO _sprint_participant_route_bindings_deepseek (
    binding_id,
    participant_id,
    route_revision,
    contract_version,
