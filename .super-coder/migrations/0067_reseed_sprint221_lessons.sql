-- 0067 — reseed: sprint-221 lessons — headless background-task discipline,
-- measurement + queue hygiene, fixture resource lifecycle.
--
-- The dos-arch sprint-221 report: six stalls from suites/benches parked on
-- session-scoped harness background tasks, a bench decided by contaminated
-- local numbers, a 4h wedge from a close-after-assert probe leak, and a
-- fleet-wide boot failure misread as an engine fault.
--
-- - sprint: new "Local long work" section — ./sc job fire-and-wake +
--   wait-slice (drain inbox between slices), --timeout on anything that can
--   wedge; measurements are CI-vs-CI on one runner. Stance + description.
-- - sprint_orchestration: inbox watcher is interactive-seat only (headless
--   boots arm nothing); Unblock gains spend-limit triage + CI queue
--   hygiene (cancel superseded verifies, log measurement-protecting
--   cancellations); stance bans hand-rolled waiters.
-- - test_authoring: rule 9 — cleanup lives in the fixture, never after the
--   asserts (leaks exactly when tests catch bugs); Never list entry.
--
...
-- forks and fresh builds alike. Requires the sc job runner (same engine
-- version) for the ./sc job references to resolve.
