-- 0082 — reseed: provider-neutral wake operations workflow (sprint 25
-- seq 10, task #86) — the operator surfaces shipped with the Interface
-- wake ops layer (sc sprint status / alerts / retry + the Interface
-- Sprint wake panel).
--
-- - sprint_orchestration: new "Wake operations" section — arm the
--   binding before the first wake, monitor `sc sprint status`, read the
--   deduped-while-open alerts (the only window into wake failures),
--   retry a park as a NEW gated batch (the parking invariant: a parked
--   batch is NEVER resubmitted; a parked input needs the operator's
--   delivered/not_delivered verdict), quarantine drains by hand, and a
--   clean close (status: CLOSED / freeze releases bindings + cancels
--   queued wake work).
-- - sprint: participant-side "Wake ops" view — read status/alerts
--   before reporting a stall; parks and quarantines go to the planner,
--   never a hand-rolled resubmission. Stance bullet.
--
