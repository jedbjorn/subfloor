-- 0118 — reseed the sprint_* skill chain: mark-read discipline, action
-- receipts, one watch registrant, recover's missing rows, dead/ungranted
-- references removed, ruling-request routing.
-- Sprint 84 U5 (spec #76, H-17..H-22; folds H-15's dev-side branch
-- declaration). Generated from assets/skills/<name>/SKILL.md; full-body
-- upsert so existing installations converge on the asset.
--
-- Covers the WHOLE chain as it stands at this head, so it is idempotent over
-- 0112 (U6's single-skill close reseed) rather than re-editing it. 0112's
-- header calls this migration 0114; the number is read back from the
-- migrations directory at file creation (decision #88) and 0114/0115 were
-- never taken -- 0117 was the last on main.
