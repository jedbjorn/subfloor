...
- [`self_update`](skills_sc/self_update.md) — Update this fork's super-coder engine in place — pull new code + migrations, keep all your memory. The shell hands off to its own next boot. Use when a super-coder update is available.
...
- [`surface_catalogue`](skills_sc/surface_catalogue.md) — Read the host repo via the dr_* catalogue (files, languages, deps, env) BEFORE grepping or walking the tree. Run `./sc map` to refresh it. Use to orient in an unfamiliar repo fast.
