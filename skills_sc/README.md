...
- [`review`](skills_sc/review.md) — Reviewer procedure — read a diff against its spec through the review lenses, open flags for failures, and ALWAYS message the author dev with findings (flags or clean). The reviewer's top-level loop; the lenses live in the skills it points to. Load when reviewing a dev's work.
...
- [`snapshot`](skills_sc/snapshot.md) — Persist DB work to git-tracked text — when and how to run ./sc snapshot / ./sc render. The .db is a cache; text is the source of truth. Snapshot is durability; the GUI Publish button commits + PRs it.
- [`spec`](skills_sc/spec.md) — Execute a spec across sessions — analyze viability, surface blockers and unclear items, break into tasks (Preparation → impl steps → Verification), and track progress in spec_tasks. Updates current_state at every step. Load when starting any feature spec.
...
- [`test_authoring`](skills_sc/test_authoring.md) — Standards for writing and reviewing stringent pytest tests — tests that can actually fail. Use when authoring or reviewing any test under `tests/`.
